...
Um balanceador de carga ou *load balancer* é uma ferramenta/técnica utilizada para redirecionar o acesso de múltiplos clientes a um determinado recurso disponível no servidor, evitando que este sofra com perda de desempenho ou inatividade por conta de um alto tráfego de dados ou por excesso de requisições. Ele atua como um intermediário recebendo todas as requisições dos clientes e redirecionando-as a um conjunto de nós (*nodes*) conectados ao balanceador de carga, distribuindo a carga entre eles de forma inteligente por meio de algoritmos como o *Round-Robin*.
...
Primeiramente, iremos realizar o *build* da nossa aplicação React por meio do `npm run build`. Para esse tutorial, irei utilizar o projeto React feito pelo professor [Rhavy Maia](https://github.com/rhavymaia):
...
Após a execução, você terá um diretório `dist/` (ou `build/`, dependendo da configuração do projeto) contendo todos os arquivos estáticos compilados da sua aplicação React. Este diretório será copiado para dentro dos contêineres Docker dos nós que executarão a aplicação. Você verá algo similar a isso:
...
Com o projeto pronto, agora podemos focar nos arquivos de configuração do Nginx. Fora do contêiner, vamos criar um arquivo `default.conf` para configurar o servidor com o `upstream`. Este arquivo define os nós que receberão as requisições. Em um ambiente Docker com múltiplos contêineres, você pode referenciar os nós de diferentes formas: usando os nomes dos contêineres em uma rede Docker, ou usando seus endereços IP. No exemplo abaixo, utilizamos os endereços IP dos contêineres Docker:
...
Com os arquivos de configuração prontos, agora podemos ir para a criação dos contêineres. Antes de criar qualquer contêiner, precisamos estabelecer uma rede Docker para que o load balancer consiga se comunicar com os nós através dos IPs internos (como `172.21.0.3:80`). Crie a rede Docker com:

```bash
docker network create load-balancer-network
```

Agora vamos criar um contêiner principal que será o balanceador de carga; nada mais justo que chamá-lo de `load-balancer`. Este contêiner terá a porta 8080 mapeada e também terá os arquivos de configuração que escrevemos mapeados como volumes. É interessante manter esses arquivos de configuração restritos à apenas leitura com o parâmetro `:ro`. Além disso, o contêiner precisa estar conectado à rede Docker que criamos. O comando fica assim:

```bash
docker run -d --name load-balancer --network load-balancer-network -p 8080:80 \
  -v /caminho/para/nginx.conf:/etc/nginx/nginx.conf:ro \
  -v /caminho/para/default.conf:/etc/nginx/conf.d/default.conf:ro \
  nginx:latest
```

Agora iremos criar os nossos nós. O processo de criação é semelhante, mas agora iremos enumerar as portas sequencialmente para que possamos identificá-los com mais facilidade. Serão esses nós que vão servir o conteúdo presente em `dist/`. Cada nó também deve estar conectado à mesma rede Docker:

```bash
docker run -d --name react-node-1 --network load-balancer-network -p 3001:80 \
  -v /caminho/para/dist:/usr/share/nginx/html:ro \
  nginx:latest

docker run -d --name react-node-2 --network load-balancer-network -p 3002:80 \
  -v /caminho/para/dist:/usr/share/nginx/html:ro \
  nginx:latest

docker run -d --name react-node-3 --network load-balancer-network -p 3003:80 \
  -v /caminho/para/dist:/usr/share/nginx/html:ro \
  nginx:latest

docker run -d --name react-node-4 --network load-balancer-network -p 3004:80 \
  -v /caminho/para/dist:/usr/share/nginx/html:ro \
  nginx:latest

docker run -d --name react-node-5 --network load-balancer-network -p 3005:80 \
  -v /caminho/para/dist:/usr/share/nginx/html:ro \
  nginx:latest
```
...
```
v@hp-256r-g9:~/Projetos/Faculdade/LoadBalancer/test$ docker ps --filter "name=react-node\|name=load-balancer" --format "table {{.Names}}\t{{.Image}}\t{{.Ports}}\t{{.Status}}"
NAMES            IMAGE          PORTS                               STATUS
load-balancer    nginx:latest   0.0.0.0:8080->80/tcp                Up 23 seconds
react-node-1     nginx:latest   0.0.0.0:3001->80/tcp                Up 15 seconds
react-node-2     nginx:latest   0.0.0.0:3002->80/tcp                Up 12 seconds
react-node-3     nginx:latest   0.0.0.0:3003->80/tcp                Up 9 seconds
react-node-4     nginx:latest   0.0.0.0:3004->80/tcp                Up 5 seconds
react-node-5     nginx:latest   0.0.0.0:3005->80/tcp                Up 1 second
```

Com tudo pronto, agora podemos realizar os nossos testes utilizando o `curl`:

```bash
# Teste básico - acessar via load balancer
curl http://localhost:8080

# Teste de um nó específico
curl http://localhost:3001

# Teste de roteamento React Router
...
Em um outro terminal, vamos utilizar o comando `docker logs` para monitorar todas as requisições feitas ao `load-balancer`. Como resultado teremos essa saída:
...
E como esperado, todas as requisições são redirecionadas igualmente. Perceba que os endereços após o `to:` variam de `172.21.0.3` a `172.21.0.7`, ou seja, cinco nós, exatamente a quantidade que definimos anteriormente.

>[!NOTE]
>Ao realizar as requisições, é possível que algumas solicitações continuem sendo encaminhadas ao mesmo nó. Isso se dá por conta da persistência de conexão ou **Keep Alive** do protocolo HTTP; o cliente `curl` reutiliza a mesma conexão TCP para economizar processamento.

## Conclusão

Neste tutorial, exploramos como configurar um balanceador de carga com Nginx para distribuir requisições entre múltiplos nós executando uma aplicação React. Começamos com conceitos fundamentais sobre load balancing, algoritmos de distribuição e a diretiva `proxy_set_header` para identificação de clientes. Em seguida, implementamos uma solução prática utilizando Docker, criando uma rede isolada onde o load balancer orquestra requisições para cinco nós independentes.
